...
impl GraphClient {
    ...
    pub fn unauthenticated(cloud: Cloud, api_version: &str) -> Result<Self, MogError> {
        Self::new(String::new(), cloud, api_version, 3, 100, false)
    }
    ...
    pub async fn request(
        ...
        loop {
            ...
            if !options.query_params.is_empty() {
                let pairs: Vec<(&str, &str)> = options.query_params
                    .iter()
                ...
                Ok(response) => {
                    ...
                    if let Some(sunset) = response.headers().get("Sunset") {
                        if let Ok(date) = sunset.to_str() {
                            eprintln!("Warning: This API endpoint is deprecated. Sunset date: {}", date);
                        }
                    }
                        ...
                        s if retry::is_retryable_status(s.as_u16()) => {
                            if attempt < self.retry_config.max_retries {
                                ...
                                warn!(
                                    ...
                                    s.as_u16(), delay, attempt + 1, self.retry_config.max_retries
                                );
                                tokio::time::sleep(delay).await;
                                attempt += 1;
                                continue;
                            ...
                            return Err(MogError::General(format!(
                                ...
                                s.as_u16(), self.retry_config.max_retries,
                                extract_error_message(&body)
                            )));
                        }
                        _ => {
    ...
    /// GET with automatic pagination, collecting up to max_results
    pub async fn get_collection(
        ...
        let limit = if all { u32::MAX } else { self.max_results };

        // Add $top to options
        let mut opts = options.clone();
        if let Some(top_val) = top {
            opts.query_params.insert("$top".to_string(), top_val.to_string());
        }
        ...
        while let Some(link) = next_link {
            ...
            if all && results.len() > 1000 {
                eprintln!("Warning: Collection exceeds 1,000 results. Use --top to limit.");
            }
...
mod tests {
    ...
    fn test_extract_error_message() {
        ...
        assert_eq!(extract_error_message(body), "ErrorAccessDenied: Access is denied.");
    }
